...
class FontWrap(object):
    ...
    def renderWordsInside(self, width, words, antialias, color, background=None):
        ...
        if background is None:
            drawn_lines = [self.font.render(line, antialias, color).convert_alpha(sharedstate.screen) for line in lines]
        else:
            drawn_lines = [self.font.render(line, antialias, color, background).convert(sharedstate.screen) for line in lines]
