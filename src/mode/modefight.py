...
class ModeFight(ModeButtons):
    ...
    def _input(self, event):
        ...
        if self._result:
            if (event.type == pygame.MOUSEBUTTONUP and event.button == 1) \
                ...
                self._stopMixer()
                jovialengine.game.getInstance().state.fight_results.append(self._RESULT_SAVING[self._result])
    ...
    def _setActionDisplay(self, text: str):
        self._action_display.appendleft(
            jovialengine.game.getInstance().font_wrap.renderInside(
                200,
                text,
                constants.TEXT_COLOR,
                constants.WHITE
        ...
        self._action_display_latest = jovialengine.game.getInstance().font_wrap.renderInside(
            200,
            text,
            constants.BLACK,
            constants.WHITE
    ...
    def _endStuff(self, result_display: str):
        if len(self._player_mon.anims) < 2 and self._result_displayed < 1:
            ...
            self._action_display_latest2 = jovialengine.game.getInstance().font_wrap.renderInside(
                200,
