...
class ModeLevelUp(ModeOpening, abc.ABC):
    ...
    def __init__(self):
        ...
        jovialengine.game.getGame().state.protag_mon.setImage()
        # set up first sprite
        self._first_sprite = pygame.sprite.DirtySprite()
        self._first_sprite.image = jovialengine.game.getGame().state.protag_mon.image
        self._first_sprite.rect = jovialengine.game.getGame().state.protag_mon.rect
        self._first_sprite.rect.center = (constants.SCREEN_SIZE[0] // 2, constants.SCREEN_SIZE[1] * 2 // 3)
        # level up and set up second sprite
        jovialengine.game.getGame().state.protag_mon.levelUp()
        jovialengine.game.getGame().state.protag_mon.setImage()
        jovialengine.game.getGame().state.protag_mon.rect.midbottom = self._first_sprite.rect.midbottom
        self._all_sprites.add(self._first_sprite, jovialengine.game.getGame().state.protag_mon)
        jovialengine.game.getGame().state.protag_mon.visible = 0
        ...
        try:
            ...
            pass
        file_name = f"{jovialengine.game.getGame().state.protag_mon.name}_{jovialengine.game.getGame().state.protag_mon.uuid}.png"
        file_path = os.path.join(constants.IMAGE_DIRECTORY, file_name)
        pygame.image.save(jovialengine.game.getGame().state.protag_mon.getCard(), file_path)
    ...
    def _switchVisibleSprite(self):
        ...
        if self._first_sprite.visible:
            ...
            jovialengine.game.getGame().state.protag_mon.visible = 1
        else:
